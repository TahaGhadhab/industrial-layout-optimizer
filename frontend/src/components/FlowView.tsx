  ...
  if (!res) {
    return (
      <div className="animate-fade-in flex flex-col items-center justify-center h-64 text-[var(--color-text-muted)] gap-3 bg-[var(--color-surface-light)] rounded-xl border border-[var(--color-surface-lighter)]">
        ...
        <p>Run <strong>2. Generate Initial Layout</strong> to analyze edge crossings.</p>
      </div>
    );
  }
    ...
    <div className="animate-fade-in max-w-3xl mx-auto">
      ...
      {crossingsCount === 0 ? (
        <div className="p-8 rounded-xl bg-[rgba(16,185,129,0.05)] border border-[rgba(16,185,129,0.2)] text-center text-[var(--color-success)] animate-slide-in flex flex-col items-center gap-3">
          ...
          <p className="text-sm opacity-80">No intersecting material flows detected. The layout is planar.</p>
        </div>
      ) : (
        <div className="space-y-3">
          <h4 className="text-sm font-bold text-[var(--color-danger)] flex items-center gap-2 mb-3 tracking-wide">
            ...
            {crossingFlows.map((cf: any, i: number) => (
              <div key={i} className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6 p-4 rounded-xl bg-[var(--color-surface-light)] border border-[var(--color-surface-lighter)] hover:border-[var(--color-danger)] transition-colors group">
                ...
                <div className="flex-1 flex items-center justify-center gap-3 p-3 rounded-lg bg-[rgba(255,255,255,0.02)] border border-[rgba(255,255,255,0.05)]">
                  <span className="px-3 py-1 bg-[var(--color-surface)] text-[var(--color-primary-light)] font-bold rounded shadow-sm border border-[var(--color-surface-lighter)]">{cf.edge1[0]}</span>
                  <span className="text-[var(--color-text-muted)] text-sm">↔</span>
                  <span className="px-3 py-1 bg-[var(--color-surface)] text-[var(--color-primary-light)] font-bold rounded shadow-sm border border-[var(--color-surface-lighter)]">{cf.edge1[1]}</span>
                </div>
                ...
                {/* Edge 2 */}
                <div className="flex-1 flex items-center justify-center gap-3 p-3 rounded-lg bg-[rgba(255,255,255,0.02)] border border-[rgba(255,255,255,0.05)]">
                  <span className="px-3 py-1 bg-[var(--color-surface)] text-[var(--color-accent)] font-bold rounded shadow-sm border border-[var(--color-surface-lighter)]">{cf.edge2[0]}</span>
                  <span className="text-[var(--color-text-muted)] text-sm">↔</span>
                  <span className="px-3 py-1 bg-[var(--color-surface)] text-[var(--color-accent)] font-bold rounded shadow-sm border border-[var(--color-surface-lighter)]">{cf.edge2[1]}</span>
                </div>
          ...
          <div className="mt-6 p-4 rounded-lg bg-[rgba(59,130,246,0.1)] border border-[rgba(59,130,246,0.2)] text-[var(--color-text-muted)] text-xs leading-relaxed flex items-start gap-3">
            <span className="text-blue-400 text-lg mt-0.5">💡</span>
            <p>
              Crossings represent points where material handling paths intersect, increasing congestion and collision risk. Try running <strong className="text-white">Optimization</strong> to swap machines and reduce these intersections while maximizing the $R_O$ (Optimality Ratio).
            </p>
          </div>
        </div>
      )}
